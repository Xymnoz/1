...
let deletingProfession = false;

function bioTyping(){

    const current = professions[professionIndex];

    if(!deletingProfession){

        typingBio.textContent =
        ...
        if(characterIndex > current.length){
            ...
            deletingProfession = true;

            setTimeout(bioTyping, 2000);

            return;

        }

        setTimeout(bioTyping, 70);
        ...
        if(characterIndex < 0){
            ...
            setTimeout(bioTyping, 500);

            return;

        }

        setTimeout(bioTyping, 110);
